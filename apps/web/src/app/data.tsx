...
export type Screen =
  | "dashboard-mgmt" | "dashboard-role"
  | "reservation-grid" | "new-reservation" | "reservation-search" | "waitlist" | "rate-management" | "group-bookings" | "cancellation"
  | "check-in" | "check-out" | "in-house-guests" | "arrivals" | "departures" | "guest-profiles" | "room-assignment" | "walk-in" | "folio" | "key-card" | "room-access" | "key-card-log" | "pin-management"
  | "hk-board" | "hk-tasks" | "hk-schedule" | "hk-inspection" | "lost-found" | "linen-supplies" | "dnd-log"
  | "work-orders" | "asset-register" | "preventive-schedule" | "vendor-contacts"
  | "pos-terminal" | "kitchen-display" | "table-management" | "menu-management" | "dining-reservations" | "room-service" | "room-charges"
  | "internal-chat" | "guest-messaging" | "announcements" | "shift-handover"
  | "finance-folio" | "invoices" | "daily-summary" | "accounts-payable" | "revenue-reports"
  | "stock-dashboard" | "products" | "suppliers" | "stock-transactions" | "purchase-orders"
  | "staff-directory" | "roles-permissions" | "attendance" | "shift-scheduler" | "payroll"
  | "branch-overview" | "branch-comparison" | "sync-status"
  | "rp-occupancy" | "rp-revenue" | "rp-department" | "rp-guest" | "rp-inventory" | "rp-staff"
  | "user-management" | "system-health" | "device-management" | "backup-restore" | "audit-log"
  | "hotel-config" | "settings-sync" | "my-preferences" | "door-lock-settings"
  | "reservation-detail" | "guest-profile-detail" | "work-order-detail" | "staff-profile";

// ─── Role-Based Menu Visibility (Blueprint §2.5) ───────────────────────────
// Which roles see which screen in the sidebar. Each department sees only
// what it needs to operate day-to-day; MGT/ORG see virtually everything for
// oversight. A handful of screens are deliberately narrower than "whole
// department" -- e.g. Front Desk keeps Housekeeping Board (needs to know
// which rooms are ready) and Work Orders (needs to report issues) without
// seeing the rest of Housekeeping/Maintenance, and MGT/ORG are excluded
// from hands-on operational tools they'd never use directly (My Tasks
// tablet view, the live POS/Kitchen Display terminals) in favor of the
// oversight screens in the same department. This is server-enforced too
// (`requireRole` on every route) -- hiding the nav entry is UX, not the
// security boundary.
// RO (Resident Officer) and CS (Customer Service) are in the Blueprint's
// matrix and already enforced server-side, but no login role in this app
// can currently be RO/CS (no seed account, not assignable via HR/IT invite
// yet), so they're omitted here -- add them if/when those roles ship.
export const SCREEN_ROLE_MAP: Partial<Record<Screen, Role[]>> = {
  "dashboard-mgmt": ["MGT", "ORG"],
  "dashboard-role": ["MGT", "ORG", "FD", "HK", "MX", "FIN", "RT", "RSV", "IT"],

  "reservation-grid": ["FD", "RSV", "MGT", "ORG"],
  "new-reservation": ["FD", "RSV", "MGT", "ORG"],
  "group-bookings": ["RSV", "MGT", "ORG"],
  "waitlist": ["FD", "RSV", "MGT", "ORG"],
  "rate-management": ["RSV", "FIN", "MGT", "ORG"],
  "reservation-search": ["FD", "RSV", "MGT", "ORG"],
  "cancellation": ["FD", "RSV", "MGT", "ORG"],

  "check-in": ["FD", "MGT", "ORG"],
  "check-out": ["FD", "MGT", "ORG"],
  "room-assignment": ["FD", "RSV", "MGT", "ORG"],
  "guest-profiles": ["FD", "RSV", "MGT", "ORG"],
  "in-house-guests": ["FD", "MGT", "ORG"],
  "arrivals": ["FD", "RSV", "MGT", "ORG"],
  "departures": ["FD", "MGT", "ORG"],
  "folio": ["FD", "FIN", "MGT", "ORG"],
  "walk-in": ["FD", "MGT", "ORG"],
  "key-card": ["FD", "IT", "MGT", "ORG"],
  "room-access": ["FD", "MGT", "ORG"],
  "key-card-log": ["FD", "IT", "MGT", "ORG"],
  "pin-management": ["FD", "MGT", "ORG"],

  "hk-board": ["FD", "HK", "MGT", "ORG"],
  "hk-tasks": ["HK"],
  "hk-schedule": ["HK", "MGT", "ORG"],
  "hk-inspection": ["HK", "MGT", "ORG"],
  "lost-found": ["HK", "MGT", "ORG"],
  "linen-supplies": ["HK", "FIN", "MGT", "ORG"],
  "dnd-log": ["FD", "HK", "MGT", "ORG"],

  "work-orders": ["FD", "HK", "MX", "MGT", "ORG"],
  "asset-register": ["MX", "IT", "MGT", "ORG"],
  "preventive-schedule": ["MX", "MGT", "ORG"],
  "vendor-contacts": ["MX", "FIN", "MGT", "ORG"],

  "pos-terminal": ["RT"],
  "kitchen-display": ["RT"],
  "table-management": ["RT", "MGT", "ORG"],
  "menu-management": ["RT", "MGT", "ORG"],
  "dining-reservations": ["RT", "MGT", "ORG"],
  "room-service": ["FD", "RT", "MGT", "ORG"],
  "room-charges": ["FD", "RT", "FIN", "MGT", "ORG"],

  "internal-chat": ["MGT", "ORG", "FD", "HK", "MX", "FIN", "RT", "RSV", "IT"],
  "guest-messaging": ["FD", "MGT", "ORG"],
  "announcements": ["MGT", "ORG"],
  "shift-handover": ["FD", "RSV", "HK", "MX", "RT", "MGT", "ORG"],

  "finance-folio": ["FD", "FIN", "MGT", "ORG"],
  "invoices": ["FD", "RT", "FIN", "MGT", "ORG"],
  "daily-summary": ["FIN", "MGT", "ORG"],
  "accounts-payable": ["FIN", "MGT", "ORG"],
  "revenue-reports": ["FIN", "MGT", "ORG"],

  "stock-dashboard": ["HK", "MX", "RT", "FIN", "MGT", "ORG"],
  "products": ["FIN", "MGT", "ORG"],
  "suppliers": ["MX", "FIN", "MGT", "ORG"],
  "stock-transactions": ["HK", "MX", "RT", "FIN", "MGT", "ORG"],
  "purchase-orders": ["FIN", "MGT", "ORG"],

  "staff-directory": ["MGT", "ORG"],
  "roles-permissions": ["IT", "MGT", "ORG"],
  "attendance": ["FIN", "MGT", "ORG"],
  "shift-scheduler": ["MGT", "ORG"],
  "payroll": ["FIN", "MGT", "ORG"],

  "branch-overview": ["ORG"],
  "branch-comparison": ["ORG"],
  "sync-status": ["IT", "ORG"],

  "rp-occupancy": ["RSV", "FIN", "MGT", "ORG"],
  "rp-revenue": ["FIN", "MGT", "ORG"],
  "rp-department": ["MGT", "ORG", "FD", "HK", "MX", "FIN", "RT", "RSV", "IT"],
  "rp-guest": ["RSV", "MGT", "ORG"],
  "rp-inventory": ["HK", "RT", "FIN", "MGT", "ORG"],
  "rp-staff": ["FIN", "MGT", "ORG"],

  "user-management": ["IT", "MGT", "ORG"],
  "system-health": ["IT", "ORG"],
  "device-management": ["IT"],
  "backup-restore": ["IT", "ORG"],
  "audit-log": ["IT", "ORG"],

  "hotel-config": ["IT", "MGT", "ORG"],
  "door-lock-settings": ["IT", "MGT", "ORG"],
  "settings-sync": ["IT", "ORG"],
  "my-preferences": ["MGT", "ORG", "FD", "HK", "MX", "FIN", "RT", "RSV", "IT"],
};

// Door Lock (Phase 4) isn't its own top-level NAV section -- it's spread
// across Front Desk's key-card/room-access/pin-management/key-card-log
// children plus one Settings entry -- so NAV_MODULE_MAP's "one section =
// one module" filter in App.tsx can't gate it. This does, per-screen,
// alongside SCREEN_ROLE_MAP: master-disabling the module (Blueprint 6.11)
// genuinely hides these from the sidebar and Settings menu, not just the
// Check-In Step 7 UI.
export const DOOR_LOCK_SCREENS: Screen[] = ["key-card", "room-access", "key-card-log", "pin-management", "door-lock-settings"];
