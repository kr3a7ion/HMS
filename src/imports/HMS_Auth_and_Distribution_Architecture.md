...
**Companion to:** HMS Complete Master Blueprint v1.0
...
When a receptionist opens Chrome on the front desk PC, they do not go to `https://yourdomain.com`. They go to a local address — `http://hms.local` or `http://192.168.1.10` — and the local server responds with the complete React application, validates their credentials, issues their session token, and processes every subsequent request. Internet is not involved in any part of this.
...
│  URL: admin.yourplatform.com (public internet)                   │
...
│  URL: portal.yourplatform.com (public internet)                  │
...
│  URL: http://hms.local (hotel LAN only)                         │
...
Browser → `https://admin.yourplatform.com` → served by the central cloud server.

## 3.3 Login Flow

```
┌─────────────────────────────────────────────────────────────────┐
│  1. Navigate to admin.yourplatform.com                          │
...
│  2. Navigate to portal.yourplatform.com                         │
...
│                                                                  │
│  What works offline:                                            │
│  ✅ All core HMS operations (reservations, check-in, billing,   │
...
│  Contents:                                                        │
│    {                                                             │
│      "iss": "hms-local-branch-{branch_id}",                     │
│      "sub": "{user_id}",                                         │
│      "role": "front_desk",                                       │
│      "org_id": "{org_id}",                                      │
│      "branch_id": "{branch_id}",                                 │
...
5. Email contains a time-limited reset link (1 hour expiry)
   pointing to: http://hms.local/reset-password?token={token}
...
5. Super Admin uses the portal (`portal.yourplatform.com`) to reset their central password.
...
```
Device (browser) → http://hms.local → Local Server → App loads
```
...
2. Open browser.
3. Navigate to `http://hms.local` (or the LAN IP, e.g. `http://192.168.1.10`).
...
Add `192.168.1.10  hms.local` to the device's hosts file. Requires one-time admin access per device.

Option B — Local DNS on the hotel router:
Configure the router's DNS to resolve `hms.local` → local server IP. Applies to every device on the network automatically. Preferred approach.
...
```
chrome.exe --app=http://hms.local --start-fullscreen
```

This makes the HMS feel like a native installed application — no browser chrome, no tabs, no address bar visible. Staff simply see the HMS at full screen when the PC boots.
...
3. Pull the HMS local server image from your private registry:
   docker pull registry.yourplatform.com/hms-local:latest
...
4. Run the provisioning command with the branch's unique
   ...
   docker run -d \
     ...
     -v hms_data:/data \
     registry.yourplatform.com/hms-local:latest
...
Hotel LAN
│
├── Front Desk PC 1     → http://hms.local (Receptionist A)
├── Front Desk PC 2     → http://hms.local (Receptionist B)
├── Manager's Laptop    → http://hms.local (Branch Manager)
├── Housekeeping Tablet → http://hms.local (HK Attendant)
├── Maintenance Tablet  → http://hms.local (Technician)
├── Restaurant Terminal → http://hms.local (Restaurant Staff)
└── Accountant's PC     → http://hms.local (Accountant)
```
...
1. You push a new Docker image to your private registry:
   docker push registry.yourplatform.com/hms-local:1.2.3
...
Tag strategy in Docker registry:
  ...
  hms-local:stable       → All branches pull this by default
  hms-local:beta         → Specific branches configured to pull beta
  hms-local:1.2.3        → Pin a specific version (emergency rollback)

To stage a rollout:
  1. Push hms-local:beta
  2. Configure 2–3 test branches to pull the beta channel
  3. Monitor for 24–48 hours via Admin Console
  4. If stable: docker tag hms-local:beta hms-local:stable
...
Branch apps (`http://hms.local`) are only accessible on the hotel LAN. This is intentional — the offline-first architecture is predicated on LAN-only operation. There is no public URL for a branch app by default.
...
The Super Admin's cross-branch remote access is handled by the Organization Portal at `portal.yourplatform.com` — a separate, internet-facing interface served by the central server showing aggregated data from all branch syncs. This gives remote visibility without exposing any branch's local server.
...
- Remote user connects VPN → appears to be on hotel LAN → browses to `http://hms.local` normally
...
─────────────────────────────────────────────────────
4. Log into admin.yourplatform.com.
...
19. Configures LAN hostname (hms.local → machine IP).
...
| Is there an app to install on client devices? | No — browser only, navigate to http://hms.local |
...
*End of document.*
*Companion document to: HMS Complete Master Blueprint v1.0*
